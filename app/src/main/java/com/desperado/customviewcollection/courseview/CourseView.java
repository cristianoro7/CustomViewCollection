...
public class CourseView extends View {
    ...
    private void initAttr(AttributeSet attributeSet) {
        ...
        mMorningCourseCounts = array.getInteger(R.styleable.CourseView_morningLessonCount, 4);
    ...
    private void drawWeekList(Canvas canvas) {
        ...
        for (int i = 1; i <= mWeekList.length; i++) {
            ...
            canvas.drawText(mWeekList[i - 1], (mColumnWidth * (i + 1) - rect.width() / 2 - mColumnWidth / 2),
                    (rect.height() / 2 + mWeekListHeight / 2), mTextPaint); //画周几
        }
    }
    ...
    private void drawRecBg(Canvas canvas) {
        ...
        canvas.drawRect(new Rect(0, mWeekListHeight,
                mColumnWidth, mWeekListHeight + mCoursePadding * 2 +
                mMorningCourseHeight), mTodayPaint); //画上午课程的红色背景

        canvas.drawRect(new Rect(0, mWeekListHeight +
                mMorningCourseHeight + mCoursePadding * 2,
                mColumnWidth, mWeekListHeight + mCoursePadding * 2 +
                mMorningCourseHeight + mAfternoonHeight + mCoursePadding * 2), mTodayPaint); //画下午课程红色背景
    }
    ...
    private void drawFirstLine(Canvas canvas) {
        ...
        canvas.drawText("couse_am", mColumnWidth - rect.width() / 2, mWeekListHeight
                + rect.height() / 2, mAmAndPmPaint);
    ...
    private void drawSecondLine(Canvas canvas) {
        canvas.drawLine(0, mWeekListHeight + mMorningCourseHeight + mCoursePadding * 2,
                mColumnWidth - mCircleRadius, mWeekListHeight +
                        mMorningCourseHeight + mCoursePadding * 2, mLinePaint);
        canvas.drawCircle(mColumnWidth, mWeekListHeight + mMorningCourseHeight + mCoursePadding * 2,
        ...
        canvas.drawText(pm, mColumnWidth - rect.width() / 2, mWeekListHeight
                + mMorningCourseHeight + mCoursePadding * 2 + rect.height() / 2, mAmAndPmPaint);
        canvas.drawLine(mColumnWidth + mCircleRadius, mWeekListHeight +
                mMorningCourseHeight + mCoursePadding * 2, mWidth, mWeekListHeight +
                mMorningCourseHeight + mCoursePadding * 2, mLinePaint);
    }
    ...
    private void drawMorningLessonText(Canvas canvas, String lesson) {
        ...
        for (int i = 0; i < mMorningCourseCounts; i++) {
            ...
            canvas.drawText(lesson, mColumnWidth / 2 - (lessonRect.width() + numberRect.width()) / 2, mWeekListHeight
                            + mCoursePadding + (singleMorningCourseHeight * i + singleMorningCourseHeight / 2 + lessonRect.height() / 2),
                    mTextPaint);
            mTextPaint.setTextSize(mLessonNumberTextSize);
            canvas.drawText(sectionLength[i], mColumnWidth / 2 + (lessonRect.width() - (lessonRect.width() + numberRect.width()) / 2), mWeekListHeight
                    + mCoursePadding + (singleMorningCourseHeight * i + singleMorningCourseHeight / 2 + lessonRect.height() / 2), mTextPaint);
        }
    }
    ...
    private void drawAfternoonLessonText(Canvas canvas, String lesson) {
        ...
        for (int i = 0; i < mAfternoonCourseCounts; i++) {
            ...
            mTextPaint.setTextSize(mLessonTextSize);
            canvas.drawText(lesson, mColumnWidth / 2 - (lessonRect.width() + numberRect.width()) / 2, mWeekListHeight + mMorningCourseHeight +
                            mCoursePadding * 3 +
                            (singleAfternoonCourseHeight * i + singleAfternoonCourseHeight / 2 + lessonRect.height() / 2),
                    mTextPaint);

            mTextPaint.setTextSize(mLessonNumberTextSize);
            canvas.drawText(afterSectionLength[i], mColumnWidth / 2 + (lessonRect.width() - (lessonRect.width() + numberRect.width()) / 2),
                    mWeekListHeight + mMorningCourseHeight +
                            mCoursePadding * 3 +
                            (singleAfternoonCourseHeight * i + singleAfternoonCourseHeight / 2 + lessonRect.height() / 2), mTextPaint);
        }
    }
    ...
    private void drawRedCourseLine(Canvas canvas) {
        ...
        canvas.drawLine(mColumnWidth, mWeekListHeight
                ...
                mWeekListHeight + mCoursePadding * 2
                        + mMorningCourseHeight - mCircleRadius, mLinePaint); //画第一个红线

        canvas.drawLine(mColumnWidth, mWeekListHeight + mCoursePadding * 2 + mMorningCourseHeight
                        + mCircleRadius,
                mColumnWidth, mWeekListHeight + mCoursePadding * 2 + mMorningCourseHeight
                        + mAfternoonHeight + mCoursePadding * 2,
                mLinePaint);
    }
    ...
    private void drawNormalLine(Canvas canvas, String course, int singleMorningCourseHeight,
        ...
        canvas.drawLine(mColumnWidth * (i + 1), mWeekListHeight
                        + mCoursePadding + singleMorningCourseHeight / 2 - rect.height() / 2,
                mColumnWidth * (i + 1), mWeekListHeight
                        + mMorningCourseHeight + mCoursePadding - rect.width() / 2, mTextPaint);

        canvas.drawLine(mColumnWidth * (i + 1), mWeekListHeight +
                        mCoursePadding * 2 + mMorningCourseHeight + mCoursePadding + singleAfternoonCourseHeight / 2 - rect.height() / 2,
                mColumnWidth * (i + 1),
                mWeekListHeight + mCoursePadding * 2 + mMorningCourseHeight
                        + mCoursePadding + mAfternoonHeight - (singleAfternoonCourseHeight / 2 - rect.width() / 2 + dip2px(5)), mTextPaint);
    }
    ...
    private void drawCourse(Canvas canvas, int singleMorningCourseHeight, int singleAfternoonCourseHeight,
        ...
        if (sectionIndex <= mMorningCourseCounts) {
            canvas.drawText(course, mColumnWidth * weekIndex + mColumnWidth / 2 - rect.width() / 2, mWeekListHeight
                            + mCoursePadding + (singleMorningCourseHeight * (sectionIndex - 1) + singleMorningCourseHeight / 2 + rect.height() / 2),
                    mTextPaint);
        } else {
            canvas.drawText(course, mColumnWidth * weekIndex + mColumnWidth / 2 - rect.width() / 2,
                    mWeekListHeight + mMorningCourseHeight + mCoursePadding * 3 +
                            (singleAfternoonCourseHeight * (sectionIndex - mMorningCourseCounts - 1) + singleAfternoonCourseHeight / 2 + rect.height() / 2), mTextPaint);
        }
    }
